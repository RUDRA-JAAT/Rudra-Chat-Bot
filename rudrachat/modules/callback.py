...
from rudrachat.database import RUDRA
...
async def cb_handler(_, query: CallbackQuery):
    if query.data == "HELP":
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_RUDRA = RUDRA.find_one({"chat_id": query.message.chat.id})
            if not is_RUDRA:
                await query.edit_message_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ.**")
            if is_RUDRA:
                RUDRA.delete_one({"chat_id": query.message.chat.id})
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_RUDRA = RUDRA.find_one({"chat_id": query.message.chat.id})
            if not is_RUDRA:
                RUDRA.insert_one({"chat_id": query.message.chat.id})
                await query.edit_message_text(
